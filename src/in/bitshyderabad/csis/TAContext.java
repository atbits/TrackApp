...
class TAContext{
	...
	final private int DEBUG_SPEED = 1; // should be 1 by default
	...
	void doSampling(){
		...
		if((nonZeroSampleCount > (NumSamples/3)) && ((mAvgRX + mAvgTX) >(1.2 * RX_TX_TSH)) && (!isStreaming)){
			isStreaming = true;
			Log.d(MY_TAG,"STREAMING START");
		} else if(isStreaming && ((nonZeroSampleCount < (NumSamples/4)) || ((mAvgRX + mAvgTX) < (0.8 * RX_TX_TSH)))){
			isStreaming = false;
			Log.d(MY_TAG,"STREAMING END : To Do Ask User");
		}
		...
		if(null != locationManager){
			sIC.add(totBytes,get_network(),isStreaming,text,getCurrentLocation());
		} else {
			sIC.add(totBytes,get_network(),isStreaming,text);
		}
	}
	...
	boolean shouldCreateLogFile() 	{
		if (isStreaming) 
			return (sAC.getCount() + sIC.getCount() > 4 * NUMBER_NODES);
		else
			return (sAC.getCount() + sIC.getCount() > NUMBER_NODES);
	}
	
	int getNextPollDelay(){
		if(isStreaming) return FAST_SAMPLE;
	...
	class SpeedInfoCollector {
		class SpeedEntry {
			...
			SpeedEntry(long stats, String txt, String opr, String date, String loc, boolean streaming){
				statCount = stats; text = txt; operator = opr; 
				time = date; location = loc; isStreaming = streaming; 
			}
		}
		ServiceAppCollector sAC = null;
		...
		void add(long statCount, String netOperator, boolean streaming, String metricsDump, String location){
			...
			entries.add(new SpeedEntry(statCount,metricsDump, netOperator, svcRef.getCurrDate(),location,isStreaming));
		}

		void add(long statCount, String netOperator,boolean streaming, String metricsDump){
			add(statCount,netOperator, streaming, metricsDump, null);
		}
		...
		void appendXmlEntries(XmlSerializer xSer) throws IOException{
			for(int idx =0; idx < entries.size(); idx++)
			{
				...
				if(sE.isStreaming) xSer.attribute(xSer.getNamespace(), "streaming", "true");
				xSer.text(sE.text);
